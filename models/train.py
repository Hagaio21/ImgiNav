...
def main():
    ...
    if args.resume and args.resume.exists():
        ...
        # Try to load training history from logs
        logs_path = Path(args.resume).parent / Path(args.resume).name.replace("checkpoint_", "logs_")
        if logs_path.exists():
            ...
            # Try to load full training history if available
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if should_save:
            ...
            logs_path = output_dir / f"logs_epoch_{epoch + 1:03d}.json"
            ...
            if val_loader and "val_loss" in epoch_log and epoch_log["val_loss"] == best_val_loss:
                ...
                best_logs_path = output_dir / "logs_best.json"
        ...
        latest_logs_path = output_dir / "logs_latest.json"
    ...
    print(f"\nTraining complete!")
    print(f"  Checkpoints (with config): {output_dir}/checkpoint_*.pt")
    print(f"  Logs (loss and eval dicts): {output_dir}/logs_*.json")
