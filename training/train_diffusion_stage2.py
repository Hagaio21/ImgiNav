...
def train_epoch(model, dataloader, scheduler, noise_loss_fn, semantic_loss_fn, optimizer, device, epoch, use_amp=False, max_grad_norm=None):
    ...
    # Ensure decoder is trainable (unfreeze if needed)
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if use_amp and device_obj.type == "cuda":
            with torch.amp.autocast('cuda'):
                ...
                noise_target = noise
                noise_loss = ((pred_noise - noise_target).pow(2) * w).mean()
                
                # Decode latents for semantic losses
...
def save_samples(model, val_loader, device, output_dir, epoch, sample_batch_size=4, exp_name=None):
    ...
    num_steps = model.scheduler.num_steps
    print(f"  Generating {sample_batch_size} samples using DDIM ({num_steps} steps)...")
    
    with torch.no_grad():
        sample_output = model.sample(
            batch_size=sample_batch_size,
            num_steps=50,  # Faster sampling for Stage 2
            method="ddim",
            eta=0.0,
            device=device_obj,
            verbose=False
...
def main():
    ...
    if stage1_checkpoint and not should_resume:
        ...
        if hasattr(model, 'decoder'):
            print("Unfreezing decoder for Stage 2 training...")
            for param in model.decoder.parameters():
                param.requires_grad = True
    ...
    noise_loss_cfg = config["training"]["loss"].get("noise_loss", {"type": "MSELoss", "key": "pred_noise", "target": "noise", "weight": 1.0})
    noise_loss_fn = LOSS_REGISTRY[noise_loss_cfg["type"]].from_config(noise_loss_cfg)
    
    semantic_loss_cfg = config["training"]["loss"].get("semantic_loss", None)
    semantic_loss_fn = None
    if semantic_loss_cfg:
        semantic_loss_fn = LOSS_REGISTRY[semantic_loss_cfg["type"]].from_config(semantic_loss_cfg)
        print(f"  Semantic loss: {semantic_loss_cfg['type']}")
    else:
        print("  No semantic loss configured")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loader and (epoch + 1) % sample_interval == 0:
            save_samples(model, val_loader, device_obj, output_dir, epoch + 1, sample_batch_size=4, exp_name=exp_name)
