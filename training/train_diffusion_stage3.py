...
def train_epoch(model, dataloader, scheduler, noise_loss_fn, semantic_loss_fn, discriminator, discriminator_weight, optimizer, device, epoch, use_amp=False, max_grad_norm=None):
    ...
    # Keep decoder frozen - we only want to update the UNet
    # The decoder should decode any valid latent, and if it can't decode the UNet's latents,
    # that's a signal that the UNet is generating bad latents (which we want to fix)
...
def save_samples(model, val_loader, device, output_dir, epoch, sample_batch_size=64, exp_name=None):
    ...
    with torch.no_grad():
        sample_output = model.sample(
            ...
            num_steps=ddim_steps,  # 100 steps for better quality
            method="ddim",
            eta=0.0,
            device=device_obj,
            verbose=False
