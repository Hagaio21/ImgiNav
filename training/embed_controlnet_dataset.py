...
def embed_layouts_with_vae(
    ...
    # Load VAE from checkpoint with strict=False to handle spatial projection layer mismatches
    # We only need encoder/decoder for embedding, not CLIP projections
    print(f"Loading VAE from checkpoint...")
    autoencoder = Autoencoder.load_checkpoint(checkpoint_path, map_location="cpu", strict=False)
