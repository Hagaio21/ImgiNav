...
from training.visualization import create_image_grid, create_comparison_grid, latents2rgb
...
def compute_metrics(model, val_loader, device, config, num_samples=50, ddim_steps=50):
    ...
    print(f"  Computing metrics with {num_samples} samples...")
    ...
    with torch.no_grad():
        for batch_idx in range(num_batches):
            ...
            if hasattr(model, 'embedding_proj') and model.embedding_proj is not None:
                ...
                if use_text_emb and text_emb is None:
                    text_emb = torch.zeros((batch_size_actual, 384), device=device_obj, dtype=param_dtype)
                elif not use_text_emb:
                    text_emb = None
                
                if use_pov_emb and pov_emb is None:
                    pov_emb = torch.zeros((batch_size_actual, 512), device=device_obj, dtype=param_dtype)
                elif not use_pov_emb:
                    pov_emb = None
            
            # Generate images
            gen_output = model.sample(
                batch_size=target_latents.shape[0],
                latent_shape=target_latents.shape[1:],
...
def save_samples(model, val_loader, device, output_dir, epoch, sample_batch_size=16, exp_name=None, guidance_scale=1.0, cfg_dropout_rate=0.0, config=None):
    ...
    # Load embeddings for all conditions we'll use
    # If we need more samples than conditions, we'll repeat conditions
    text_emb_list = []
    pov_emb_list = []
    for i in range(batch_size):  # Load all available conditions
        sample_dir = conditioned_dir / f"sample_{i}"
        conditions_dir = sample_dir / "conditions"
    ...
    if text_emb_list:
        ...
        # Skip if it's all zeros (was a placeholder for missing condition)
            ...
            # Repeat conditions to get exactly total_samples (1 sample per condition)
            # Cycle through available conditions if needed
            text_emb_repeated = []
            for i in range(total_samples):
                condition_idx = i % batch_size  # Cycle through conditions
                text_emb_repeated.append(text_emb_single[condition_idx:condition_idx+1])
    ...
    if pov_emb_list:
        ...
        # Skip if it's all zeros (was a placeholder for missing condition)
            ...
            for i in range(total_samples):
                condition_idx = i % batch_size  # Cycle through conditions
                pov_emb_repeated.append(pov_emb_single[condition_idx:condition_idx+1])
    ...
    # Load target images for comparison grids (use first min(batch_size, total_samples) conditions)
    target_images = []
    num_targets_to_load = min(batch_size, total_samples)
    for i in range(num_targets_to_load):
        sample_dir = conditioned_dir / f"sample_{i}"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if should_eval:
            ...
            computed_metrics = compute_metrics(
                ...
                device_obj, 
                config, 
                num_samples=50,  # Reduced sample size for faster computation
                ddim_steps=50
            )
            # Add all computed metrics to val_logs
