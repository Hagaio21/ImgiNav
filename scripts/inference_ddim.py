#!/usr/bin/env python3
"""
DDIM inference script for diffusion experiments.

This script loads a diffusion experiment and generates images using DDIM sampling.
"""
...
def generate_image(model, autoencoder, device: str, num_steps: int = 50) -> Image.Image:
    """Generate an image using DDIM sampling."""
    model.eval()
    autoencoder.eval()
    ...
    with torch.no_grad():
        ...
        if hasattr(model, 'sample'):
            ...
            print(f"DDIM generation time: {generation_time:.2f} seconds ({generation_time/num_steps:.3f} seconds per step)")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='Generate images from diffusion experiments using DDIM')
    ...
    parser.add_argument('--output', type=str, default='ddim_generated.png', help='Output image path')
    parser.add_argument('--num_steps', type=int, default=50, help='Number of sampling steps (default: 50 for DDIM)')
    ...
    # Use specified steps or default for DDIM
    num_steps = args.num_steps
    print(f"Using DDIM with {num_steps} steps")
    
    # Generate image
    print(f"Generating image using DDIM with {num_steps} steps...")
    ...
    print(f"DDIM image saved to: {output_path}")
