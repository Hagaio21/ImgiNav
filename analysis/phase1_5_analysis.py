#!/usr/bin/env python3
"""
Analysis script for Phase 1.5: Final Training
Loads metrics and creates final evaluation visualizations.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze Phase 1.5 final training")
    parser.add_argument(
        "--phase-dir",
        type=str,
        default="outputs/phase1_5_final_training",
    ...
    print("=" * 80)
    print("Phase 1.5: Final Training Analysis")
    print("=" * 80)
    ...
    print(f"\nLoaded {len(exp_names)} experiment(s)\n")
    ...
    # Visual comparison with actual model
    if not args.skip_visual:
        print("\n" + "-" * 80)
        print("Loading autoencoder checkpoint for final evaluation...")
        print("-" * 80)
        checkpoints = load_autoencoder_checkpoints(exp_names, args.experiments_dir)
        
        if checkpoints:
            print("\nLoading test samples from dataset...")
            test_scenes, test_rooms = get_test_samples(args.dataset_manifest, test_samples_per_type=3)
            ...
            print("  Could not load checkpoint, skipping visual comparison")
    
    print("\n" + "=" * 80)
    print("Final Training Analysis complete!")
