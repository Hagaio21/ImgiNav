...
def build_graph_index(graphs_dir: Path) -> Tuple[Dict[str, str], Dict[str, str]]:
    ...
    if jsons_dir.exists():
        graphs_dir_name = graphs_dir.name  # "pov_graphs" or "graphs"
        for path in jsons_dir.glob("*_room_graph.json"):
            ...
            if not re.search(r'(door|window)\d+', name):
                json_index[name] = f"{graphs_dir_name}/jsons/{path.name}"
    ...
    if texts_dir.exists():
        ...
        for path in texts_dir.glob("*_room_description.txt"):
            ...
            if not re.search(r'(door|window)\d+', name):
                text_index[name] = f"{graphs_dir_name}/texts/{path.name}"
...
def build_pov_graph_index(graphs_dir: Path) -> Dict[Tuple[str, str, str], Tuple[str, str]]:
    ...
    for path in jsons_dir.glob("*_room_graph.json"):
        ...
        if scene_id and room_id:
            # Use correct path based on directory name
            graphs_dir_name = graphs_dir.name  # "pov_graphs" or "graphs"
            json_path = f"{graphs_dir_name}/jsons/{path.name}"
            text_path = f"{graphs_dir_name}/texts/{name}_room_description.txt"
...
def collect_manifest_data_pov_normalized(
    ...
    # Try new location first (pov_graphs/), fallback to old (graphs/)
    graphs_dir = dataset_root / "pov_graphs"
    if not graphs_dir.exists():
        logger.info(f"  pov_graphs/ not found, trying graphs/...")
        graphs_dir = dataset_root / "graphs"
    
    logger.info(f"  Using graphs directory: {graphs_dir}")
