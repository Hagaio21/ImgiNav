...
# Usage: bash run_stage4_array.sh <shard_id>
...
STAGE5_SCRIPT="${BASE_DIR}/data_preparation_v2/hpc_scripts/run_stage5_array.sh"
...
echo "=========================================="
echo "Stage 4: Render POVs - Shard ${JOB_ID}"
...
# Run stage 4 with scene list and HPC mode
...
echo "Stage 4 completed successfully for shard ${JOB_ID}"

# Submit stage 5 with the same shard
echo "Submitting stage 5 for shard ${JOB_ID}..."
if [ -n "${PARENT_JOB_ID}" ]; then
  # Wait for parent job if we have one
  bsub -J "stage5_shard${JOB_ID}" \
       -o "${LOG_DIR}/stage5_shard${JOB_ID}.%J.out" \
       -e "${LOG_DIR}/stage5_shard${JOB_ID}.%J.err" \
       -n 2 \
       -R "rusage[mem=4000]" \
       -W 2:00 \
       -q hpc \
       -w "ended(${PARENT_JOB_ID})" \
       bash "${STAGE5_SCRIPT}" "${JOB_ID}"
else
  bsub -J "stage5_shard${JOB_ID}" \
       -o "${LOG_DIR}/stage5_shard${JOB_ID}.%J.out" \
       -e "${LOG_DIR}/stage5_shard${JOB_ID}.%J.err" \
...
echo "Stage 5 job submitted for shard ${JOB_ID}"
echo "Stage 4 complete for shard ${JOB_ID}"
