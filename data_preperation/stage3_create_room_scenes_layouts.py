...
def create_room_layout(parquet_path: Path, output_path: Path, taxonomy_path: Path,
    ...
    for unique_id in np.unique(filtered_labels):
        color = palette.get(int(unique_id), (128, 128, 128))
        mask = filtered_labels == unique_id
        color_array = np.array(color, dtype=np.uint8)
        
        for x, y in zip(x_img[mask], y_img[mask]):
            draw_point(canvas, x, y, color_array, size=point_size)
...
def create_scene_layout(scene_dir: Path, output_path: Path, taxonomy_path: Path,
    ...
    # Load semantic info for floor detection
    ...
    # Collect global bounds using floor points (or all points as fallback)
    all_u_bounds, all_v_bounds = [], []
    
    for parquet_path in room_parquets:
        try:
            df = pd.read_parquet(parquet_path, columns=["x", "y", "z", "label_id"])
            xyz = df[["x", "y", "z"]].to_numpy(dtype=np.float32)
            
            # Use floor points if available, otherwise all points
            if floor_ids and "label_id" in df.columns:
                floor_mask = df["label_id"].isin(floor_ids)
                if floor_mask.any():
                    xyz = xyz[floor_mask]
            
            uvh = world_to_local_coords(xyz, origin, u, v, n)
            all_u_bounds.extend([float(uvh[:, 0].min()), float(uvh[:, 0].max())])
            all_v_bounds.extend([float(uvh[:, 1].min()), float(uvh[:, 1].max())])
            
        except Exception as e:
            print(f"[warn] failed to process {parquet_path} for bounds: {e}")
    
    if not all_u_bounds:
        print(f"[warn] no usable points found in {scene_dir}")
        return
    
    global_uv_bounds = (min(all_u_bounds), max(all_u_bounds), 
                       min(all_v_bounds), max(all_v_bounds))
    ...
    for parquet_path in room_parquets:
        try:
            ...
            # Apply height filtering
            height_mask = np.ones(len(xyz), dtype=bool)
            if height_min is not None:
                height_mask &= uvh[:, 2] >= height_min
            if height_max is not None:
                height_mask &= uvh[:, 2] <= height_max
            
            u_vals, v_vals = uvh[height_mask, 0], uvh[height_mask, 1]
            filtered_labels = labels[height_mask]
            
            x_img, y_img = points_to_image_coords(u_vals, v_vals, global_uv_bounds, 
                                                 resolution, margin)
...
def main():
    ...
    ap.add_argument("--res", type=int, default=768, help="Output image resolution")
    ...
    if args.mode in ("room", "both"):
        ...
        if not room_files:
            print("No room parquets found")
        else:
            print(f"Generating {len(room_files)} room layouts...")
            ...
            for i, parquet_path in enumerate(room_files, 1):
                ...
                if len(parts) >= 2:
                    ...
                    scene_id = parquet_path.parents[2].name
                    room_id = parquet_path.parent.name
                
                layout_name = f"{scene_id}_{room_id}_room_seg_layout.png"
                output_path = parquet_path.parent / "layouts" / layout_name
                ...
                try:
                    ...
                    progress(i, f"{parquet_path.name} ({elapsed:.2f}s) -> {output_path}", True)
                except Exception as e:
    ...
    if args.mode in ("scene", "both"):
        ...
        if not scene_ids:
            ...
            print(f"Generating {len(scene_ids)} scene layouts...")
